...
public class LoginTests extends BaseTest {
	...
	public void testInvalidLogin(String username, String password, String expected) {
		if (!"failure".equalsIgnoreCase(expected)) return;

		LoginPage lp = new LoginPage(driver);
		lp.login(username, password);

		boolean landedOnOverview = lp.waitForAccountsOverview(3);
		String err = lp.waitForErrorMessage(5);
		boolean stillOnLogin = lp.isStillOnLoginPage();

		Assert.assertFalse(landedOnOverview, "Unexpectedly landed on Accounts Overview with invalid credentials.");
		Assert.assertTrue(stillOnLogin || (err != null && !err.isBlank()),
				"Expected to remain on login page or see an error message.");
	}*/
	@Test(priority = 2, dataProvider = "loginData", dataProviderClass = TestDataProvider.class)
