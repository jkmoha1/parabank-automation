...
public class UpdateProfilePage extends BasePage {
    @FindBy(linkText="Update Contact Info") private WebElement updateContactLink;
    ...
    @FindBy(css="input[value='Update Profile']") private WebElement updateBtn;
    @FindBy(css="#rightPanel .title") private WebElement confirmationTitle;

    public UpdateProfilePage(WebDriver driver){ super(driver); PageFactory.initElements(driver, this); }

    public void open(){ updateContactLink.click(); }
    public void updateAddress(String a, String c, String s, String z, String p){
        type(address,a); type(city,c); type(state,s); type(zip,z); type(phone,p); click(updateBtn);
    }
