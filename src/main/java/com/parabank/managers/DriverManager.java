...
public class DriverManager {
    ...
    public static void initDriver(String browserFromTestNgParam) {
        ...
        // Resolve browser: JVM -Dbrowser > TestNG param > config.properties > chrome
        ...
        // Optional pins + binary overrides
        ...
        String chromeBinary = blankToNull(ConfigReader.get("chrome.binary"));   // e.g. C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe
        String ffBinary     = blankToNull(ConfigReader.get("firefox.binary"));  // e.g. C:\\Program Files\\Mozilla Firefox\\firefox.exe
        ...
        try {
            switch (b) {
                case "firefox" -> {
                    WebDriverManager wdm = WebDriverManager.firefoxdriver()
                            .avoidBrowserDetection()     // skip flaky local detection on some Windows images
                            .clearResolutionCache()      // avoid stale cache
                            .avoidExport();              // don't mutate PATH env
                    if (pinnedGecko != null) {
                    ...
                    if (ffBinary != null) {
                        ...
                        System.setProperty("webdriver.firefox.bin", ffBinary); // helps older combos
                    }
                    if (headless) fo.addArguments("-headless");
                    // a couple of stability prefs (safe no-ops on most machines)
                ...
                case "chrome" -> {
                    ...
                    if (pinnedChrome != null) {
                        ...
                        // Try normal detection; if it fails, skip detection
                        try { wdm.setup(); }
                        catch (Exception ex) {
                    ...
                    // If setup above didn’t run due to pin branch, ensure setup()
            ...
            e.printStackTrace(); // show the real source (WDM vs driver vs binary)
            throw e;
        }
    }
